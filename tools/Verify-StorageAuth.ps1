...
# Configuration
$functionAppName = "groupchangefunction"
$resourceGroup = "groupchangefunction"
$storageAccountName = "groupchangefunction1"
$subscriptionId = "8a89e116-824d-4eeb-8ef4-16dcc1f0959b"
$expectedClientId = "0ed597a6-5cca-4c6f-b51e-10510010e936"
$expectedPrincipalId = "e4ad71a2-53c3-467f-a553-bc7eebf711b5"
...
try
{
    ...
    if ($appInsightsId)
    {
        Write-Host "  ℹ️  Application Insights is configured. Check the Azure Portal for recent 403 errors." -ForegroundColor Cyan
...
{
    Write-Host "  ⚠️  Could not check Application Insights configuration" -ForegroundColor Yellow
}
...
try
{
    ...
    if ($queues -eq "outqueue")
    {
        ...
        {
            Write-Host "  ℹ️  Queue is empty (no recent function executions or logs)" -ForegroundColor Cyan
        }
    }
    else
    {
        Write-Host "  ⚠️  Queue 'outqueue' not found or not accessible" -ForegroundColor Yellow
    }
}
catch
{
    Write-Host "  ⚠️  Could not access queue storage. Error: $($_.Exception.Message)" -ForegroundColor Yellow
    Write-Host "     This might be a permissions issue for YOUR account, not the managed identity" -ForegroundColor Gray
}

# Summary
Write-Host "`n=== Verification Summary ===" -ForegroundColor Cyan
Write-Host "If all tests show ✅, the configuration is correct."
