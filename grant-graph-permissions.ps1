...
foreach ($permission in $requiredPermissions)
{
    ...
    if ($null -eq $role)
    {
        Write-Host "⚠️  Warning: Could not find permission '$($permission.Name)'" -ForegroundColor Yellow
    }
    else
    {
...
{
    Write-Host "`n❌ No valid permissions found to grant" -ForegroundColor Red
    exit 1
}
...
{
    Write-Host "⚠️  Could not check existing assignments: $($_.Exception.Message)" -ForegroundColor Yellow
    $existingRoleIds = @()
}
...
{
    Write-Host "`n✅ The managed identity now has the required permissions!" -ForegroundColor Green
    ...
    Write-Host "3. Test the auto-renewal function" -ForegroundColor White
}
else
{
    Write-Host "`n⚠️  No permissions were granted. Please check the errors above." -ForegroundColor Yellow
}
